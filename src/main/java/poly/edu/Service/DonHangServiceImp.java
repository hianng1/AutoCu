...
public class DonHangServiceImp implements DonHangService { // Đảm bảo implements DonHangService
	...
	public DonHang save(DonHang order) {
		// Logic nghiệp vụ thêm/cập nhật (nếu có) có thể viết ở đây trước khi gọi
		// repo.save
	...
	// Ví dụ: Phương thức nghiệp vụ phức tạp hơn có thể dùng kết hợp nhiều thao tác
	// từ Repository
	// @Transactional
	// public DonHang processOrderUpdate(Long orderId, TrangThai newStatus) {
	// DonHang order = findById(orderId).orElseThrow(() -> new
